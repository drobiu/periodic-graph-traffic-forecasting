...
class GCN(nn.Module):
    ...
    and outputs a tensor of predictions for each node in the graph. The model consists of one or more GCNConv layers,
    ...
    def __init__(self, node_dim, edge_dim, output_dim=1, hidden_dim=50, n_gnn_layers=1, K=2, dropout_rate=0.5):
        ...
        if n_gnn_layers == 1:
            self.convs.append(GCNConv(node_dim, output_dim))
        else:
    ...
    def forward(self, data):
        """Applies the GCN to the input graph.
