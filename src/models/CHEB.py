...
class ChebNet(nn.Module):
    ...
    and outputs a tensor of predictions for each node in the graph. The model consists of one or more ChebConv layers,
    ...
    """
    def __init__(self, node_dim, edge_dim, output_dim=1, hidden_dim=50, n_gnn_layers=1, K=2, dropout_rate=0.5):
        super().__init__()
        ...
        if n_gnn_layers == 1:
            self.convs.append(ChebConv(node_dim, output_dim, K, normalization=normalization))
        else:
            self.convs.append(ChebConv(node_dim, hidden_dim, K, normalization=normalization))
            self.prelus.append(nn.PReLU())
            self.norms.append(BatchNorm(hidden_dim))

            for l in range(n_gnn_layers-2):
                self.convs.append(ChebConv(hidden_dim, hidden_dim, K, normalization=normalization))
                self.prelus.append(nn.PReLU())
                self.norms.append(BatchNorm(hidden_dim))

            self.convs.append(ChebConv(hidden_dim, output_dim, K, normalization=normalization))
